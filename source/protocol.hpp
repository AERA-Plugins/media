...
inline bool Valid(const Message &message, bool from_worker) {
  ...
  if (from_worker)
    return (message.kind == Kind::kFrame && message.x == kWidth && message.y == kHeight &&
            message.value == kFrameBytes) || message.kind == Kind::kStatus ||
           message.kind == Kind::kError || message.kind == Kind::kEnded;
